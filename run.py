...
from hdx.scraper.fts import FTS, Locations
    ...
    with Download(
        ...
        with HDXErrorHandler(write_to_hdx=False) as error_handler:
            hapi_output = HAPIOutput(
                configuration, error_handler, fts.reqfund.global_rows, today, folder
            )
            dataset = hapi_output.generate_dataset()
            dataset.update_from_yaml(
                path=join("src/hdx/scraper/fts/config", "hdx_hapi_dataset_static.yaml")
            )
            dataset.create_in_hdx(
                remove_additional_resources=True,
...
if __name__ == "__main__":
    facade(
        ...
        project_config_yaml=join(
            "src/hdx/scraper/fts/config", "project_configuration.yaml"
        ),
    )
